...
The mapping is documented exhaustively in the [State Assembly specification](/webv2/specs/07-state-assembly/), which defines both `composeV3StateFromGraph()` (backward-compatible reassembly) and `composeV4StateFromGraph()` (new ID-keyed format).
