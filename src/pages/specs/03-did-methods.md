...
**Status:** Recommended approach decided; open for industry consensus.
...
**Recommended approach: DID-bound credential with platform-managed proof of control (Option 3).**
...
- **Platform-managed keys** — the account holder does not need to manage cryptographic keys directly. The platform holds the key in KMS and signs on behalf of the person. This preserves the zero-friction onboarding model (§10.1 of Sub-spec 06) while providing the cryptographic guarantees of self-sovereign identity.
...
**Alternative approaches (presented for industry consensus):**

| Option | Mechanism | Trust model | Key management | Limitations |
|--------|-----------|-------------|----------------|-------------|
| **1. Bearer VC presentation** | Person presents identity VC directly; no DID-binding proof | Credential signature + Federated Registry lookup | None (bearer token) | Without holder-binding, anyone with a copy of the VC can present it. Requires additional anti-replay measures (audience restriction, nonce binding). |
| **2. Firm-to-firm transfer with consent** | Person authorises Firm X to share credential with Firm Y; platform brokers the handoff | Firm-to-firm trust, both in Federated Registry | None (firm-held) | Person has no independent control over their identity. Not truly portable — tied to the originating firm's willingness to share. |
| **3. DID-bound + platform-managed proof of control** _(recommended)_ | Person's DID is bound to credential; platform proves DID control on their behalf | Cryptographic proof of control + Federated Registry | Platform-managed (KMS) | Requires platform to manage keys on behalf of users. Person trusts platform not to sign without consent. Mitigated by audit logging and future wallet migration. |

**Consensus question:** See Industry Consensus Session Q4.3.
