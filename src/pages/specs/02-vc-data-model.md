...
### 6.6 File Attachments and Access Mechanism

Conveyancing workflows rely heavily on source documents (e.g. title register PDFs, search reports). However, verifiable credentials MUST NOT embed large files directly. Instead, PDTF 2.0 uses the W3C VC standard for related resources combined with an explicit access routing hint.

**File Referencing & Integrity:**
The evidence object references the file via its `id` (URL) and secures its integrity using the W3C standard `digestMultibase` property. If the file at the URL changes, the hash will fail validation, invalidating the evidence.

**Access Protection:**
The access control on the evidence URL MUST match or exceed the `confidentiality` level of the credential itself (as defined in `PdtfAccessPolicy`). Public credentials (like EPCs) may have public evidence URLs. Restricted credentials MUST protect their evidence URLs.

**The `pdtf:access` Hint:**
To fetch a protected file, a downstream requester (like a buyer's conveyancer who holds the VC but didn't fetch it directly from the issuer) needs to know *how* to authenticate. While standard HTTP 401 challenges are one option, PDTF adds a `pdtf:access` hint to make the OID4VP flow explicit.

```json
"evidence": [{
  "type": "DocumentExtraction",
  "id": "https://adapters.propdata.org.uk/title/evidence/123.pdf",
  "digestMultibase": "uELq9FnJ5YLa5iAszyJ518bXcnlc5P7xp1u-5uJRDYKvc",
  "pdtf:access": {
    "mechanism": "OID4VP",
    "authorizationServer": "https://adapters.propdata.org.uk/title"
  }
}]
```
*(Note: Whether to use explicit hints or rely purely on HTTP 401 flows is currently an open question for industry consensus — see §13.1).*
...
2. **File handling and access negotiation.** Conveyancers need source files, but VCs shouldn't embed them. We use URLs + `digestMultibase`. How should access to these protected files be negotiated by downstream parties?
   - *Option A (Explicit routing hint):* Include a `pdtf:access` object in the VC evidence showing the exact auth server and mechanism (e.g. OID4VP).
   - *Option B (HTTP 401 Challenge):* Keep VCs clean and rely on standard OAuth/UMA patterns (GET file -> 401 Unauthorized -> WWW-Authenticate header points to auth server).
   Which approach has industry consensus for simplicity and interoperability?

3. **Credential granularity for seller forms.** When a seller fills in BASPI, should each section (heating, fixtures, legal questions) become a separate PropertyCredential, or should the entire form submission be one credential? Separate credentials allow finer-grained revocation and re-attestation. One credential is simpler to issue.
